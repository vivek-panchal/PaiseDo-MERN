...
import configData, {assignAvatarBasedOnEmail} from "../../config";

// Import the avatar assignment function from config.js
          ...
          <Grid item md={6} xs={12}>
            <AvatarGroup max={3} sx={{ width: "100%" }}>
              {groupMembers.map((member, index) => {
                const avatarUrl = assignAvatarBasedOnEmail(member);
                return (
                  <Avatar key={index} alt={member.toUpperCase()} src={avatarUrl} />
                );
              })}
            </AvatarGroup>
          </Grid>
